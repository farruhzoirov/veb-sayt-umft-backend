...
const Page = require('../../models/data/page')
...
class PageController extends DefaultController {
    ...
    async all(req, res) {
        ...
        let pages = await Page.find({ status: 0 }).sort({ _id: -1 }).limit(limit).select(select).skip(skip).lean();
        ...
        const count = await Page.find({ status: 0 }).limit(limit).count()
