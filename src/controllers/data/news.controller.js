...
class NewsController extends DefaultController {
    ...
    async all(req, res) {
        ...
        let Allnews = await NewsController.find({status: 0}).sort({_id: -1}).select(select).limit(limit).skip(skip).lean();
        ...
        const count = await NewsController.find({status: 0}).limit(limit).count()
    ...
    async get_one(req, res) {
        let language = req.query.language || 'uz';
        let Onenews = await NewsController.findOne({_id: req.params.id}).lean();
    ...
    async getOneForFront(req, res) {
        ...
        let news = await NewsController.findOne({
            slug
        }).lean()
        await NewsController.findByIdAndUpdate(news._id, {$inc: {view: 1}}, {new: true})
        let translate = await newsTranslate.findOne({key: news._id, language}).select(select).lean()
        if (translate) news = {...news, ...translate}
        let others = await NewsController.find({
            slug: {$ne: slug},
            status: 0
